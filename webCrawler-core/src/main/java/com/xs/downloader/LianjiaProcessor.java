...
public class LianjiaProcessor implements PageProcessor {
    ...
    private void buildDetails(Page page) {
        ...
        // 户型概述
//        String overview = html.xpath("//div[@class=\"dongtai-title\"]").toString();
        // 房型
        String houseStyle = html.xpath("//p[@class=\"p1\"]").toString();
        html.xpath("//*[@id=\"house-online\"]/div[1]/div[1]/ul[1]");
        ...
        String itemscore = html.xpath("//div[@class=\"itemscore\"]/text()").toString();

        // 评价内容
        String comment = html.xpath("//div[@class=\"words\"]/text()").toString();

        // 楼盘地址
        String buildingAdress = html.xpath("//span[@class=\"label-val\"]/text()").toString();
    ...
    public void process(Page page) {
        ...
        if (page.getUrl().regex(URL_LIST).match()) {
            ...
            page.putField("title", page.getHtml().xpath("//a[@class='clear']/"));
